    ...
    <div class="main-content">
        <section class="section">
            <div class="section-header">
                <h1>Laporan Analisa Air</h1>
            </div>

            <div class="section-body">
                <h2 class="section-title">Laporan Analisa Air</h2>
                            ...
                            <div class="container mt-3">
                                <div class="d-flex justify-content-between align-items-center flex-wrap">
                                    <form action="{{ url()->current() }}" method="GET"
                                        class="form-inline mb-0 d-flex align-items-center flex-wrap">
                                        ...
                                        <select name="month" id="month" class="form-control mr-4 mb-2">
                                            <option value="">Semua Bulan</option>
                                            @foreach (range(1, 12) as $bulan)
                                                <option value="{{ $bulan }}"
                                                    {{ request('month') == $bulan ? 'selected' : '' }}>
                                    ...
                                    <div class="button d-flex align-items-center" style="gap: 10px;">
                                        <a href="/operator/add_laporan_analisa_air" class="btn btn-success"
                                            style="margin-top: 0;">
                                            <i class="fa fa-plus"></i> Tambah Data
                                        </a>

                                        <a href="/operator/laporan_analisa_air/history" class="btn btn-danger"
                                            style="width:auto; text-align:center; margin:0;">
                                    ...
                                    <table class="table table-bordered table-md">
                                        ...
                                        @forelse ($laporan_analisa_air as $analisa_air)
                                            <tr>
                                                ...
                                                <td>{{ Carbon\Carbon::parse($analisa_air->tgl_sampling)->translatedFormat('d F Y') }}
                                                </td>
                                                <td align="center">
                                                    @if ($analisa_air['statusOP'] == 0)
                                                        <form
                                                            action="/operator/laporan_analisa_air/ttd/{{ $analisa_air['id'] }}"
                                                    ...
                                                    @elseif($analisa_air['statusOP'] == 1)
                                                        <img src="data:image/png;base64, {!! base64_encode(
                                                            QrCode::format('png')->size(70)->generate($analisa_air->user_id_OP . '_' . $analisa_air->name_id_OP),
                                                        ) !!}"
                                                            alt="">
                                                    @endif
                                                ...
                                                <td>
                                                    ...
                                                    @elseif($analisa_air['statusST'] == 1)
                                                        {{-- {!! QrCode::size(70)->generate($analisa_air->user_id_ST ."_". $analisa_air->name_id_ST) !!} --}}
                                                        <img src="data:image/png;base64, {!! base64_encode(
                                                            QrCode::format('png')->size(70)->generate($analisa_air->user_id_ST . '_' . $analisa_air->name_id_ST),
                                                        ) !!}"
                                                            alt="">
                                                ...
                                                <td>
                                                    @if ($analisa_air['statusSP'] == 0)
                                                        {{-- <form action="/supervisor/mikrobiologi_analisa_air/ttd/{{$analisa_air['id']}}" method="POST" class="text-center">
                                                        @csrf
                                                        @method('PATCH')
                                                    ...
                                                    @elseif($analisa_air['statusSP'] == 1)
                                                        ...
                                                        <img src="data:image/png;base64, {!! base64_encode(
                                                            QrCode::format('png')->size(70)->generate($analisa_air->user_id_SP . '_' . $analisa_air->name_id_SP),
                                                        ) !!}"
                                                            alt="">
                                                    @endif
                                                </td>
                                                <td align="center" style="justify-content:center;">
                                                    <form
                                                        action="{{ route('laporan_analisa_air_Delete', $analisa_air['id']) }}"
                                                        method="POST">
                                                        @csrf
                                                        @method('PATCH')
                                                        ...
                                                        <a class="fa-solid fa-pen-to-square text-success btn"
                                                            href="{{ route('edit_laporan_analisa_air', $analisa_air->id) }}"></a>

                                                        <br>
                                                        <a class="fa-solid fa-file-pdf ml-1 btn" target="_blank"
                                                            href="{{ route('OP_laporan_analisa_air_pdf', $analisa_air->id) }}"></a>

                                                        <a target="_blank"
                                                            href="{{ route('laporan_analisa_air_excel_show', $analisa_air->id) }}">
                                                        ...
                                                        <br>
                                                        <a href="{{ route('laporan_analisa_air_sampel', $analisa_air->id) }}"
                                        ...
                                        @empty
                                            <tr>
                                                <td class="text-center h5" colspan="12">Not Found</td>
                                            </tr>
                                        @endforelse
