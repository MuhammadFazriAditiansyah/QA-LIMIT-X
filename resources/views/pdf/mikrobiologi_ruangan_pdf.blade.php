...
<head>
    ...
    <title>laporan Analisa Mikrobiologi Ruangan {{ $mikrobiologi_ruangan->nodokumen }}</title>
    <style>
        @page {
            margin-left: 0.1in;
            margin-right: 0.1in;
    ...
    <table cellpadding="3" style="border:solid; width:100%;">
        ...
        <table width="98%" style="border:solid; margin:10px auto;">
            <tr>
                ...
                <td colspan="" align="left" style="border-left-style:hidden;">FFB/FRM/QA/18/09</td>
            </tr>
            <tr>
            ...
            <tr>
                <td colspan="" align="center" style="width:;">LAPORAN ANALISA MIKROBIOLOGI RUANGAN</td>
        ...
        <table class="table-none" style="border-style:hidden; margin-left:10%;">
            <tr style="border-style:none;">
                ...
                <td style="border-style:none;"> : {{ $mikrobiologi_ruangan->nodokumen }}</td>
            </tr>
            <tr>
                <td style="border-style:none;">Tanggal Inokulasi</td>
                <td style="border-style:none;"> :
                    {{ Carbon\carbon::parse($mikrobiologi_ruangan->tgl_inokulasi)->translatedFormat('d F Y') }}</td>
            </tr>
            <tr>
                <td style="border-style:none;">Tanggal Pengamatan</td>
                <td style="border-style:none;"> :
                    {{-- {{ Carbon\Carbon::parse($mikrobiologi_ruangan->tgl_pengamatan)->translatedFormat('d F Y') }}</td> --}}
                    {{ $mikrobiologi_ruangan->tgl_pengamatan }}</td>
            </tr>
        </table>
        ...
        <table class="table" border="0" width="95%" cellpadding="0" cellspacing="0"
            ...
            <tr style="">
                ...
                <td colspan="0" align="center">TPC ({{ $mikrobiologi_ruangan->satuan_tpc }})</td>
                <td rowspan="0" align="center">Yeast & Mold ({{ $mikrobiologi_ruangan->satuan_yeast_mold }})</td>
        ...
        <table border="0" width="auto" cellpadding="0" cellspacing="0"
            ...
            <tr align="center">
                ...
                <td cellpadding="5">
                    @if ($mikrobiologi_ruangan['statusOP'] == 0)
                        Belum ditandatangani
                    @elseif($mikrobiologi_ruangan['statusOP'] == 1)
                        <img src="data:image/png;base64, {!! base64_encode(
                            QrCode::format('png')->size(40)->generate($mikrobiologi_ruangan->user_id_OP . '_' . $mikrobiologi_ruangan->name_id_OP),
                        ) !!}" alt="">
                    @endif
                </td>
                <td>
                    @if ($mikrobiologi_ruangan['statusST'] == 0)
                        Belum ditandatangani
                    @elseif($mikrobiologi_ruangan['statusST'] == 1)
                        <img src="data:image/png;base64, {!! base64_encode(
                            QrCode::format('png')->size(40)->generate($mikrobiologi_ruangan->user_id_ST . '_' . $mikrobiologi_ruangan->name_id_ST),
                        ) !!}" alt="">
                    @elseif ($mikrobiologi_ruangan['statusST'] == 2)
                        Ditolak
                    @endif
                </td>
                <td>
                    @if ($mikrobiologi_ruangan['statusSP'] == 0)
                        Belum ditandatangani
                    @elseif($mikrobiologi_ruangan['statusSP'] == 1)
                        <img src="data:image/png;base64, {!! base64_encode(
                            QrCode::format('png')->size(40)->generate($mikrobiologi_ruangan->user_id_SP . '_' . $mikrobiologi_ruangan->name_id_SP),
                        ) !!}" alt="">
                    @endif
                </td>
            </tr>
            ...
            <tr align="center">
                ...
                <td colspan="" align="center">
                    @if ($mikrobiologi_ruangan->name_id_OP == null)
                        Belum dibaca
                    @elseif($mikrobiologi_ruangan->name_id_OP)
                        {{ $mikrobiologi_ruangan->name_id_OP }}
                    @endif
                </td>
                <td colspan="" align="center">
                    @if ($mikrobiologi_ruangan->statusST == 2)
                        Ditolak
                    @elseif($mikrobiologi_ruangan->name_id_ST == null)
                        Belum dibaca
                    @elseif($mikrobiologi_ruangan->name_id_ST)
                        {{ $mikrobiologi_ruangan->name_id_ST }}
                    @endif
                </td>
                <td colspan="" align="center">
                    @if ($mikrobiologi_ruangan->name_id_SP == null)
                        Belum dibaca
                    @elseif($mikrobiologi_ruangan->name_id_SP)
                        {{ $mikrobiologi_ruangan->name_id_SP }}
                    @endif
                </td>
            </tr>
            <tr align="center">
                ...
                <td colspan="" align="center">
                    @if ($mikrobiologi_ruangan->created_at_OP == null)
                        Data kosong
                    @else
                        {{ Carbon\Carbon::parse($mikrobiologi_ruangan->created_at_OP)->translatedFormat('d F Y') }}
                    @endif
                </td>
                <td colspan="" align="center">
                    @if ($mikrobiologi_ruangan->statusST == 2)
                        Ditolak
                    @elseif ($mikrobiologi_ruangan->created_at_ST == null)
                        Data kosong
                    @elseif ($mikrobiologi_ruangan->statusST == 1)
                        {{ Carbon\Carbon::parse($mikrobiologi_ruangan->created_at_ST)->translatedFormat('d F Y') }}
                    @endif
                </td>
                <td colspan="" align="center">
                    @if ($mikrobiologi_ruangan->created_at_SP == null)
                        Data kosong
                    @else
                        {{ Carbon\Carbon::parse($mikrobiologi_ruangan->created_at_SP)->translatedFormat('d F Y') }}
                    @endif
                </td>
            </tr>
        </table>
