...
<head>
    ...
    <title>Laporan Analisa Air</title>
    <style>
        @page {
            margin-left: 0.1in;
            margin-right: 0.1in;
...
<body style="padding:40px;">
            ...
            <td style="border-top-style: hidden;">
                <table width="98%" style="margin: 10px;">
                    ...
                    @foreach ($sampel_null as $sampel)
                        @php
                            $pengujian_count = $pengujian_laporan_analisa_air->where('sampel_id', $sampel->id)->count();
                        @endphp
                        <tr>
                            <td align="center" style="text-align:center;" rowspan="{{ $pengujian_count + 1 }}" colspan="2">{{ $loop->iteration }}</td>
                            <td align="center" style="text-align:center;" rowspan="{{ $pengujian_count + 1 }}" colspan="2">{{ $sampel->sampel }}</td>
                        ...
                        @foreach ($pengujian_laporan_analisa_air as $pengujian)
                            @if ($pengujian->sampel_id == $sampel->id)
                                <tr>
                                    <td align="center" colspan="2">{{ $pengujian->pengujian }}</td>
                                    <td align="center" colspan="2">{{ $pengujian->shift_1 }}</td>
                                    <td align="center" colspan="2">{{ $pengujian->shift_2 }}</td>
                                    <td align="center" colspan="2">{{ $pengujian->keterangan }}</td>
                                </tr>
                            @endif
                        @endforeach
                    @endforeach
                    ...
                    @foreach ($sampel_laporan_analisa_air as $sampel)
                        ...
                        @endphp
                        <tr>
                            <td align="center" rowspan="{{ $pengujian_count + 1 }}" colspan="2">{{ $no=$no+1 }}</td>
                            <td align="center" rowspan="{{ $pengujian_count + 1 }}" colspan="2">{{ $sampel->sampel }}</td>
                            <td align="center" colspan="2"></td>
                            <td align="center" colspan="2"></td>
                            <td align="center" colspan="2"></td>
                            <td align="center" colspan="2"></td>
    ...
    <div class="export-excel" style="padding:10px; float:right;">
        <a href="/laporan_anlisa_air/exportExcel/{{ $laporan_analisa_air->id }}" onclick="hideLogoImage()">Export</a>
    </div>
</body>
</html>
