    ...
    <div class="main-content">
        <section class="section">
            <div class="section-header">
                <h1>Analisa Mikrobiologi Personil</h1>
            </div>

            <div class="section-body">
                <h2 class="section-title">Analisa Mikrobiologi Personil</h2>
                        ...
                        <div class="card shadow">
                            ...
                            <div class="container mt-3">
                                ...
                                <form method="GET" action="{{ url()->current() }}"
                                    class="form-inline d-flex flex-wrap align-items-center mb-0">
                                    {{-- Filter Tahun (hanya tahun sekarang) --}}
                                    @php $currentYear = date('Y'); @endphp
                                    <select name="year" class="form-control mr-2 mb-2">
                                        <option value="{{ $currentYear }}"
                                            {{ request('year', $currentYear) == $currentYear ? 'selected' : '' }}>
                                            {{ $currentYear }}
                                        </option>
                                    </select>
                                ...
                                <div class="button d-flex align-items-center" style="gap: 10px;">
                                    <a href="/operator/add_mikrobiologi_personil" class="btn btn-success">
                                        <i class="fa fa-plus"></i> Tambah Data
                                    </a>

                                    <a href="/operator/mikrobiologi_personil/history" class="btn btn-danger">
                            ...
                            <div class="card-body mt-1 shadow">
                                <div class="table-responsive">
                                    <table class="table table-bordered table-md">
                                        ...
                                        @forelse ($mikrobiologi_personil as $personil)
                                            <tr>
                                                ...
                                                <td>{{ $personil->nama_produk }}</td>
                                                <td>{{ Carbon\Carbon::parse($personil->tgl_inokulasi)->translatedFormat('d F Y') }}
                                                </td>
                                                {{-- <td>{{Carbon\Carbon::parse($personil->tgl_pengamatan)->translatedFormat('d F Y')}}</td> --}}
                                                <td align="center">
                                                    @if ($personil['statusOP'] == 0)
                                                        <form
                                                            action="/operator/mikrobiologi_personil/ttd/{{ $personil['id'] }}"
                                                    ...
                                                    @elseif($personil['statusOP'] == 1)
                                                        <img src="data:image/png;base64, {!! base64_encode(
                                                            QrCode::format('png')->size(80)->generate($personil->user_id_OP . '_' . $personil->name_id_OP),
                                                        ) !!}"
                                                            alt="">
                                                    @endif
                                                ...
                                                <td>
                                                    ...
                                                    @elseif($personil['statusST'] == 1)
                                                        {{-- {!! QrCode::size(80)->generate($personil->user_id_ST ."_". $personil->name_id_ST) !!} --}}
                                                        <img src="data:image/png;base64, {!! base64_encode(
                                                            QrCode::format('png')->size(80)->generate($personil->user_id_ST . '_' . $personil->name_id_ST),
                                                        ) !!}"
                                                            alt="">
                                                ...
                                                <td>
                                                    @if ($personil['statusSP'] == 0)
                                                        {{-- <form action="/supervisor/mikrobiologi_personil/ttd/{{$personil['id']}}" method="POST" class="text-center">
                                                            @csrf
                                                            @method('PATCH')
                                                    ...
                                                    @elseif($personil['statusSP'] == 1)
                                                        {{-- {!! QrCode::size(80)->generate($personil->user_id_SP ."_". $personil->name_id_SP) !!} --}}
                                                        <img src="data:image/png;base64, {!! base64_encode(
                                                            QrCode::format('png')->size(80)->generate($personil->user_id_SP . '_' . $personil->name_id_SP),
                                                        ) !!}"
                                                            alt="">
                                                    @endif
                                                </td>
                                                <td align="center" style="justify-content:center;">
                                                    <form
                                                        action="{{ route('mikrobiologi_personil_Delete', $personil['id']) }}"
                                                        method="POST">
                                                        @csrf
                                                        {{-- @method('DELETE')  --}}
                                                        @method('PATCH')
                                                        ...
                                                        <a class="fa-solid fa-pen-to-square text-success btn"
                                                            href="{{ route('edit_mikrobiologi_personil', $personil->id) }}"></a>

                                                        <br>
                                                        <a class="fa-solid fa-file-pdf ml-1 btn" target="_blank"
                                                            href="{{ route('OP_mikrobiologi_personil_pdf', $personil->id) }}"></a>

                                                        <a target="_blank"
                                                            href="{{ route('mikrobiologi_personil_excel_show', $personil->id) }}">
                                                        ...
                                                        <br>
                                                        <a href="{{ route('mikrobiologi_personil_sampel', $personil->id) }}"
                                    ...
                                    {{ $mikrobiologi_personil->links('pagination::bootstrap-4') }}
                                </div>
                            </div>
                        </div>
                    </div>
