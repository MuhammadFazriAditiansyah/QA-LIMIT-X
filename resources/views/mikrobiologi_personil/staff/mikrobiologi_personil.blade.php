   ...
   <div class="main-content">
    <section class="section">
        <div class="section-header">
            <h1>Data Analisa Mikrobiologi Personil</h1>
        </div>

        <div class="section-body">
            <h2 class="section-title">Data Analisa Mikrobiologi Personil</h2>
                                    ...
                                    <tr>
                                        ...
                                        <td align="center">
                                            @if($personil['statusOP'] == 0)
                                                ...
                                                {{-- {!! QrCode::size(80)->generate($personil->user_id_OP ."_". $personil->name_id_OP) !!} --}}
                                            @endif
                                        </td>
                                        <td align="center">
                                            @if($personil['statusST'] == 0)
                                                <form action="/staff/mikrobiologi_personil/ttd/{{$personil['id']}}" method="POST" class="text-center">
                                                    @csrf
                                                    @method('PATCH')
                                                ...
                                                <form action="/staff/mikrobiologi_personil/declinettd/{{$personil['id']}}" method="POST" style="margin-top:5%;" class="text-center">
                                                    @csrf
                                                    @method('PATCH')
                                        ...
                                        <td align="center" style="justify-content;center;">
                                            <form action="{{route('delete', $personil['id'])}}" method="POST" style="text-center">
                                                @csrf
                                                @method('DELETE')
                                                {{-- <button class="text-danger btn"><i class="fa-solid fa-trash"></i></button>
                                                <br>
                                                <a class="fa-solid fa-pen-to-square text-success btn" href="{{route('edit', $personil->id)}}"></a>
                                                <br>--}}

                                                {{-- <a class="fa-solid fa-file-pdf ml-1 btn" target="_blank" href="{{route('staff_analisakimiapdf', $personil->id)}}"></a> --}}
                                                <a href="{{route('ST_mikrobiologi_personil_pdf', $personil->id)}}" target="_blank"><i class="fa-solid fa-file-pdf ml-1 fa-lg"></i></a>
