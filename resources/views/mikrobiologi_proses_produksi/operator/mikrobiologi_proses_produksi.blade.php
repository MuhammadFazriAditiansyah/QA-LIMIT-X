    ...
    <div class="main-content">
        <section class="section">
            <div class="section-header">
                <h1>Analisa Mikrobiologi Proses Produksi</h1>
            </div>

            <div class="section-body">
                <h2 class="section-title">Analisa Mikrobiologi Proses Produksi</h2>
                            ...
                            <div class="container mt-3">
                                <div class="d-flex justify-content-between align-items-center flex-wrap">
                                    ...
                                    <div class="d-flex flex-wrap align-items-center">
                                        <a href="/operator/add_mikrobiologi_proses_produksi"
                                            class="btn btn-success mr-2 mb-2">
                                            <i class="fa fa-plus"></i> Tambah Data
                                        </a>

                                        <a href="/operator/mikrobiologi_proses_produksi/history"
                                    ...
                                    <table class="table table-bordered table-md">
                                        ...
                                        @forelse ($mikrobiologi_proses_produksi as $proses_produksi)
                                            <tr>
                                                ...
                                                <td>{{ $proses_produksi->nama_produk }}</td>
                                                <td>{{ Carbon\Carbon::parse($proses_produksi->tgl_inokulasi)->translatedFormat('d F Y') }}
                                                </td>
                                                {{-- <td>{{Carbon\Carbon::parse($proses_produksi->tgl_pengamatan)->translatedFormat('d F Y')}}</td> --}}
                                                <td align="center">
                                                    @if ($proses_produksi['statusOP'] == 0)
                                                        <form
                                                            action="/operator/mikrobiologi_proses_produksi/ttd/{{ $proses_produksi['id'] }}"
                                                    ...
                                                    @elseif($proses_produksi['statusOP'] == 1)
                                                        <img src="data:image/png;base64, {!! base64_encode(
                                                            QrCode::format('png')->size(80)->generate($proses_produksi->user_id_OP . '_' . $proses_produksi->name_id_OP),
                                                        ) !!}"
                                                            alt="">
                                                    @endif
                                                ...
                                                <td>
                                                    ...
                                                    @elseif($proses_produksi['statusST'] == 1)
                                                        {{-- {!! QrCode::size(80)->generate($proses_produksi->user_id_ST ."_". $proses_produksi->name_id_ST) !!} --}}
                                                        <img src="data:image/png;base64, {!! base64_encode(
                                                            QrCode::format('png')->size(80)->generate($proses_produksi->user_id_ST . '_' . $proses_produksi->name_id_ST),
                                                        ) !!}"
                                                            alt="">
                                                ...
                                                <td>
                                                    @if ($proses_produksi['statusSP'] == 0)
                                                        {{-- <form action="/supervisor/mikrobiologi_proses_produksi/ttd/{{$proses_produksi['id']}}" method="POST" class="text-center">
                                                            @csrf
                                                            @method('PATCH')
                                                    ...
                                                    @elseif($proses_produksi['statusSP'] == 1)
                                                        {{-- {!! QrCode::size(80)->generate($proses_produksi->user_id_SP ."_". $proses_produksi->name_id_SP) !!} --}}
                                                        <img src="data:image/png;base64, {!! base64_encode(
                                                            QrCode::format('png')->size(80)->generate($proses_produksi->user_id_SP . '_' . $proses_produksi->name_id_SP),
                                                        ) !!}"
                                                            alt="">
                                                    @endif
                                                </td>
                                                <td align="center">
                                                    <form
                                                        action="{{ route('mikrobiologi_proses_produksi_Delete', $proses_produksi['id']) }}"
                                                        method="POST">
                                                        @csrf
                                                        {{-- @method('DELETE')  --}}
                                                        @method('PATCH')
                                                        ...
                                                        <a class="fa-solid fa-pen-to-square text-success btn"
                                                            href="{{ route('edit_mikrobiologi_proses_produksi', $proses_produksi->id) }}"></a>

                                                        <br>
                                                        <a class="fa-solid fa-file-pdf ml-1 btn" target="_blank"
                                                            href="{{ route('OP_mikrobiologi_proses_produksi_pdf', $proses_produksi->id) }}"></a>

                                                        <a target="_blank"
                                                            href="{{ route('mikrobiologi_proses_produksi_excel_show', $proses_produksi->id) }}">
                                                        ...
                                                        <br>
                                                        <a href="{{ route('mikrobiologi_proses_produksi_sampel', $proses_produksi->id) }}"
