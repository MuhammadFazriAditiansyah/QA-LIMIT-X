    ...
    <div class="main-content">
        <section class="section">
            <div class="section-header">
                <h1>Analisa Mikrobiologi Bahan Baku</h1>
            </div>

            <div class="section-body">
                <h2 class="section-title">Analisa Mikrobiologi Bahan Baku</h2>
                        ...
                        <div class="card shadow">
                            ...
                            <div class="container mt-3">
                                <div class="d-flex justify-content-between align-items-center flex-wrap">
                                    ...
                                    {{-- Tombol Tambah Data & History Delete --}}
                                    <div class="d-flex flex-wrap align-items-center">
                                        <a href="/operator/add_mikrobiologi_bahan_baku" class="btn btn-success mr-2 mb-2">
                                            <i class="fa fa-plus"></i> Tambah Data
                                        </a>
                                        <a href="/operator/mikrobiologi_bahan_baku/history" class="btn btn-danger mb-2">
                            ...
                            <div class="card-body mt-1 shadow">
                                <div class="table-responsive">
                                    <table class="table table-bordered table-md">
                                        ...
                                        @forelse ($mikrobiologi_bahan_baku as $bahan_baku)
                                            <tr>
                                                ...
                                                <td>{{ $bahan_baku->nama_produk }}</td>
                                                <td>{{ Carbon\Carbon::parse($bahan_baku->tgl_inokulasi)->translatedFormat('d F Y') }}
                                                </td>
                                                {{-- <td>{{Carbon\Carbon::parse($bahan_baku->tgl_pengamatan)->translatedFormat('d F Y')}}</td> --}}
                                                <td align="center">
                                                    @if ($bahan_baku['statusOP'] == 0)
                                                        <form
                                                            action="/operator/mikrobiologi_bahan_baku/ttd/{{ $bahan_baku['id'] }}"
                                                    ...
                                                    @elseif($bahan_baku['statusOP'] == 1)
                                                        <img src="data:image/png;base64, {!! base64_encode(
                                                            QrCode::format('png')->size(80)->generate($bahan_baku->user_id_OP . '_' . $bahan_baku->name_id_OP),
                                                        ) !!}"
                                                            alt="">
                                                    @endif
                                                </td>
                                                <td>
                                                    ...
                                                    @elseif($bahan_baku['statusST'] == 1)
                                                        {{-- {!! QrCode::size(80)->generate($bahan_baku->user_id_ST ."_". $bahan_baku->name_id_ST) !!} --}}
                                                        <img src="data:image/png;base64, {!! base64_encode(
                                                            QrCode::format('png')->size(80)->generate($bahan_baku->user_id_ST . '_' . $bahan_baku->name_id_ST),
                                                        ) !!}"
                                                            alt="">
                                                ...
                                                <td>
                                                    @if ($bahan_baku['statusSP'] == 0)
                                                        {{-- <form action="/supervisor/mikrobiologi_bahan_baku/ttd/{{$bahan_baku['id']}}" method="POST" class="text-center">
                                                                @csrf
                                                                @method('PATCH')
                                                    ...
                                                    @elseif($bahan_baku['statusSP'] == 1)
                                                        {{-- {!! QrCode::size(80)->generate($bahan_baku->user_id_SP ."_". $bahan_baku->name_id_SP) !!} --}}
                                                        <img src="data:image/png;base64, {!! base64_encode(
                                                            QrCode::format('png')->size(80)->generate($bahan_baku->user_id_SP . '_' . $bahan_baku->name_id_SP),
                                                        ) !!}"
                                                            alt="">
                                                    @endif
                                                </td>
                                                <td align="center" style="justify-content:center;">
                                                    <form
                                                        action="{{ route('mikrobiologi_bahan_baku_Delete', $bahan_baku['id']) }}"
                                                        method="POST">
                                                        @csrf
                                                        {{-- @method('DELETE')  --}}
                                                        @method('PATCH')
                                                        ...
                                                        <a class="fa-solid fa-pen-to-square text-success btn"
                                                            href="{{ route('edit_mikrobiologi_bahan_baku', $bahan_baku->id) }}"></a>

                                                        <br>
                                                        <a class="fa-solid fa-file-pdf ml-1 btn" target="_blank"
                                                            href="{{ route('OP_mikrobiologi_bahan_baku_pdf', $bahan_baku->id) }}"></a>

                                                        <a target="_blank"
                                                            href="{{ route('mikrobiologi_bahan_baku_excel_show', $bahan_baku->id) }}">
                                                        ...
                                                        <br>
                                                        <a href="{{ route('mikrobiologi_bahan_baku_sampel', $bahan_baku->id) }}"
                                    ...
                                    {{ $mikrobiologi_bahan_baku->links('pagination::bootstrap-4') }}
                                </div>
                            </div>
                        </div>
                    </div>
