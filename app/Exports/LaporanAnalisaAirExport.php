...
class LaporanAnalisaAirExport implements FromView, ShouldAutoSize, WithStyles, WithDrawings, WithEvents
{
    /**
    ...
    */
    ...
    {
        return view('Excel.laporan_analisa_air.export_laporan_analisa_air', [
            'laporan_analisa_air' => Laporan_analisa_air::where('id',$this->id)->first(),
            'sampel_laporan_analisa_air' => Sampel_laporan_analisa_air::where('id_dokumen', $this->id)->get(),
            'pengujian_laporan_analisa_air' => Pengujian_laporan_analisa_air::where('id_analisa_air', $this->id)->get(),
    ...
    public function styles(Worksheet $sheet)
    {
        ...
        // $sheet = $event->sheet;
        $lastRow = $sheet->getHighestRow(); //cell seterusnya 
        $no = 'A10:B' . $lastRow;
        $sheet->getStyle($no)->getAlignment()->setHorizontal(Alignment::HORIZONTAL_CENTER);
        $sheet->getStyle($no)->getAlignment()->setVertical(Alignment::VERTICAL_CENTER);
        $sampel = 'C10:D' . $lastRow;
        $sheet->getStyle($sampel)->getAlignment()->setHorizontal(Alignment::HORIZONTAL_CENTER);
        $sheet->getStyle($sampel)->getAlignment()->setVertical(Alignment::VERTICAL_CENTER);
        ...
        // $sheet->getStyle('M7:N7')->getAlignment()->setHorizontal(\PhpOffice\PhpSpreadsheet\Style\Alignment::HORIZONTAL_LEFT);
